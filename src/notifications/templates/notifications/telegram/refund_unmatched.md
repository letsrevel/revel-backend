{% load i18n %}**{% trans "Refund could not be matched to a ticket" %}**

{% blocktranslate with amount=context.refund_amount currency=context.currency intent=context.payment_intent_id %}A refund of {{ amount }} {{ currency }} arrived from Stripe on payment {{ intent }}, but Revel could not tell which ticket it belongs to. No ticket was cancelled and no seat was freed.{% endblocktranslate %}
