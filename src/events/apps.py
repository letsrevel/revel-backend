...
class EventsConfig(AppConfig):
    ...
    def ready(self) -> None:
        ...
        ``announcement_tasks`` modules (so the Celery worker registers their
        tasks) and installs the per-app exception handlers on the global Ninja
        API.

        Returns:
            None: Performs registration side effects only.
        """
        from . import announcement_tasks, signals, tasks_stripe  # noqa: F401
