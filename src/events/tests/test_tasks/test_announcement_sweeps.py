...
def event(org: Organization) -> Event:
    return Event.objects.create(
        organization=org, name="Sweep Event", slug="sweep-event",
...
def _ticket(event: Event, user: RevelUser) -> None:
    ...
    Ticket.objects.create(
        event=event, tier=tier, user=user, status=Ticket.TicketStatus.ACTIVE,
...
class TestSendScheduledSweep:
    def test_sends_due_absolute_and_skips_future(
        ...
        due = Announcement.objects.create(
            organization=org, event=event, title="due", body="b", created_by=owner,
        ...
        future = Announcement.objects.create(
            organization=org, event=event, title="future", body="b", created_by=owner,
            status=Announcement.AnnouncementStatus.SCHEDULED,
    ...
    def test_relative_due_is_sent(
        self, org: Organization, revel_user_factory: RevelUserFactory,
        django_capture_on_commit_callbacks: t.Any,
    ) -> None:
        # Event starts in 30 min; offset -60 min => effective send 30 min ago => due.
        ev = Event.objects.create(
            organization=org, name="Soon", slug="soon",
            event_type=Event.EventType.PUBLIC, visibility=Event.Visibility.PUBLIC,
            status=Event.EventStatus.OPEN,
        ...
        ann = Announcement.objects.create(
            organization=org, event=ev, title="rel", body="b", created_by=org.owner,
            status=Announcement.AnnouncementStatus.SCHEDULED,
...
class TestResendSweep:
    def test_resends_for_active_event_and_skips_ended(
        ...
        _ticket(event, early)
        ann = Announcement.objects.create(
            organization=org, event=event, title="loc", body="b", created_by=org.owner,
            resend_to_new_signups=True, past_visibility=True,
        )
        with django_capture_on_commit_callbacks(execute=True):
        ...
        ended_ev = Event.objects.create(
            organization=org, name="Ended", slug="ended",
            event_type=Event.EventType.PUBLIC, visibility=Event.Visibility.PUBLIC,
            status=Event.EventStatus.OPEN,
            start=timezone.now() - dt.timedelta(days=2), end=timezone.now() - dt.timedelta(days=1),
        )
        _ticket(ended_ev, revel_user_factory(username="ended_attendee"))
        ended_ann = Announcement.objects.create(
            organization=org, event=ended_ev, title="old", body="b", created_by=org.owner,
            status=Announcement.AnnouncementStatus.SENT, sent_at=timezone.now() - dt.timedelta(days=2),
            resend_to_new_signups=True, past_visibility=True, recipient_count=1,
        )
