...
def test_reactivate_race_loss_returns_409(
    ...
    url = reverse("api:reactivate_waitlist_offer", kwargs={"event_id": event.pk, "offer_id": offer.pk})
    with mock.patch(
        "events.controllers.event_admin.waitlist_offers.reactivate_admin_offer",
...
def test_reactivate_at_capacity_returns_409(
    organization_owner_client: Client,
    event: Event,
    revel_user_factory: RevelUserFactory,
) -> None:
    """Cannot reactivate an offer when the event is already at capacity."""
    from events.models import EventRSVP

    _set_window(event)
    event.max_attendees = 1
    event.requires_ticket = False
    event.save(update_fields=["max_attendees", "requires_ticket"])
    # Fill the only seat with a confirmed RSVP.
    ...
    assert response.status_code == 409
    offer.refresh_from_db()
    # Offer stays in its original (EXPIRED) state.
...
def test_create_offer_from_entry(
    ...
    body = response.json()
    assert body["status"] == WaitlistOffer.WaitlistOfferStatus.PENDING.value
...
def test_create_offer_race_loss_returns_409(
    ...
    url = reverse("api:create_waitlist_offer", kwargs={"event_id": event.pk})
    with mock.patch(
        "events.controllers.event_admin.waitlist_offers.create_admin_offer",
        side_effect=IntegrityError("duplicate pending"),
    ):
        response = organization_owner_client.post(
            url,
...
def test_create_offer_at_capacity_returns_409(
    organization_owner_client: Client,
    event: Event,
    revel_user_factory: RevelUserFactory,
) -> None:
    """Cannot manually create an offer when the event is already at capacity."""
    from events.models import EventRSVP
    ...
    assert response.status_code == 409
    assert WaitlistOffer.objects.filter(event=event, user=user).count() == 0
