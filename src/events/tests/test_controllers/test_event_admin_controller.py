...
# --- Tests for GET /event-admin/{event_id}/invitation_requests ---
...
def test_list_event_invitation_requests(
    ...
    url = reverse("api:list_event_invitation_requests", kwargs={"event_id": public_event.pk})
...
def test_list_pending_tickets_by_owner(
    organization_owner_client: Client,
    event: Event,
    ...
    """Test that organization owner can list pending offline tickets."""
    url = reverse("api:list_pending_tickets", kwargs={"event_id": event.pk})
    response = organization_owner_client.get(url)

    assert response.status_code == 200
    data = response.json()

    # Should only include pending offline/at-door tickets
    assert data["count"] == 2
...
def test_list_pending_tickets_by_staff_with_permission(
    organization_staff_client: Client,
    event: Event,
    staff_member: OrganizationStaff,
    pending_offline_ticket: Ticket,
) -> None:
    """Test that staff with manage_tickets permission can list pending tickets."""
    # Grant permission
    perms = staff_member.permissions
...
def test_list_pending_tickets_by_staff_without_permission(
    organization_staff_client: Client,
    event: Event,
    staff_member: OrganizationStaff,
    pending_offline_ticket: Ticket,
...
def test_list_pending_tickets_search(
    organization_owner_client: Client,
    event: Event,
    pending_offline_ticket: Ticket,
    pending_at_door_ticket: Ticket,
) -> None:
    """Test searching pending tickets by user email or name."""
    url = reverse("api:list_pending_tickets", kwargs={"event_id": event.pk})

    # Search by user's email
    search_email = pending_offline_ticket.user.email
    response = organization_owner_client.get(url, {"search": search_email})
...
def test_list_pending_tickets_pagination(
    organization_owner_client: Client, event: Event, offline_tier: TicketTier
) -> None:
    """Test pagination of pending tickets."""
...
def test_pending_tickets_endpoints_require_authentication(event: Event, pending_offline_ticket: Ticket) -> None:
    ...
    list_url = reverse("api:list_pending_tickets", kwargs={"event_id": event.pk})
...
def test_check_in_success(organization_owner_client: Client, event: Event, active_online_ticket: Ticket) -> None:
    ...
    url = reverse("api:check_in_ticket", kwargs={"event_id": event.pk})
    payload = {"ticket_id": str(active_online_ticket.id)}
