...
def test_refresh_updates_matching_links_only(
    ...
    link = sync_service.refresh_counts(pushed)
    by_name = {tl.tier.name: tl for tl in TierLink.objects.filter(event_link=link).select_related("tier") if tl.tier}
    ...
    assert TierLink.objects.filter(event_link=link).count() == 2  # remote-only class creates no link
...
def test_refresh_missing_marks_broken(pushed: EventLink, fake_provider: FakeProvider) -> None:
    ...
    link = sync_service.refresh_counts(pushed)
    assert link.sync_state == EventLink.SyncState.BROKEN and link.remote_id == ""
    assert not TierLink.objects.filter(event_link=link).exists()
