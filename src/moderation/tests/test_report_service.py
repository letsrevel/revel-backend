...
def test_create_report_snapshots_text(user: RevelUser) -> None:
    ...
    payload = ReportCreateSchema(
        content_type="accounts.fooditem", object_id=food.id, reason=ContentReport.Reason.OFFENSIVE
    )
...
def test_non_reportable_target_404(user: RevelUser) -> None:
    payload = ReportCreateSchema(
        content_type="accounts.globalban", object_id=uuid.uuid4(), reason=ContentReport.Reason.OFFENSIVE
    )
