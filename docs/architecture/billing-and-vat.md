...
- **Collection**: the percent sent to Stripe is the org's `platform_fee_percent` grossed up with platform VAT when applicable (`percent × (1 + rate/100)`, capped at 100) — the same fee-plus-VAT-on-top economics as tickets. Reverse-charge and non-EU orgs get the bare percent. The grossed percent is frozen into the Stripe Subscription at Checkout; a later change to the org's VAT status does not retroactively update it. This staleness also shows up on the recording side: if the org's VAT status changes after Checkout (e.g. it later becomes cross-border with a validated VAT ID), renewals keep charging the frozen grossed percent while the ledger decomposes each collected fee at the *current* rules, so the booked net can drift from the originally intended net until the subscription's fee percent is resynced.
